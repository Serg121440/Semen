...
from app.parser.telegram import UNKNOWN_DATE, extract_message, parse_channel_html
...
def test_extracts_russian_month_date_range():
    result = extract_message(
        "Горящие билеты Москва — Ереван с 25 по 28 июля, 12 900 ₽",
...
def test_parser_keeps_deal_without_dates():
    payload = extract_message("Горящие билеты в Стамбул 9 900 ₽")
    assert payload["departure_date"] is None
    assert UNKNOWN_DATE.year == 1970
